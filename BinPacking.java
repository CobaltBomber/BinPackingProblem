...
public class BinPacking {
    ...
    public static ArrayList<Bin> FirstFit(int binSize, int[] itemSizes){
        ...
        for (int i = 0; i < itemSizes.length; i++) {
            ...
            if (itemSizes[i] > binSize) { //Case that the Item is larger than the max bin size, asks user to restart
    ...
    public static Bin BestFit(int itemSize, TreeSet<Bin> tree) {
        Bin tempBin = new Bin(itemSize);
    ...
    public static TreeSet<Bin> BestFitDecreasing(int binSize, int[] itemSizes){
        ...
        for (int i = 0; i < itemSizes.length; i++) {
            if (BestFit(itemSizes[i], bestTree) == null) {
                Bin myBin = new Bin(binSize);
                myBin.AddItem(itemSizes[i]);
                bestTree.add(myBin);
            }
            else {
                Bin returnValue = BestFit(itemSizes[i], bestTree);
